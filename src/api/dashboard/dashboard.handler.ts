...
/**
 * GET /v1/dashboard — the signed-in home, shaped by the current role. Profile Contract v2:
 * `currentRole` is bff SESSION state (never a Core/DB value — the id_token carries no app role
 ...
 *
 * A session with no (or a stale) current role has no dashboard "home" of its own — the frontend
 ...
 * `me.currentRole === "GUIDE"` check this replaces.
 */
export const getDashboard = withSession(async (req, res, core) => {
  ...
  // session, so readSession(req) is guaranteed non-null; the `provisioningStatus` narrowing (rather
  ...
  const currentRole =
    session?.provisioningStatus === "PROVISIONED" ? session.currentRole : undefined;
