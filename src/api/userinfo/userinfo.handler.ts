 ...
 * `GET /users/me` with no `activeRole` at all — `activeRole` is per-SESSION bff state (see
 * `SessionData.activeRole`), so the bff is the only place that can answer this. The web app
 * calls this on every page load, so it stays a cheap read: no session write on the common path.
 *
 * Re-validates the session's `activeRole` against the roles Core JUST returned, every call
 * (defensive, read-mostly): a role the account no longer holds (revoked/suspended) — or any
 * stale/garbage value `isRole` rejects — is reported as `activeRole: null`, and ONLY THEN is
...
export const getUserinfo = withSession(async (req, res, core) => {
  ...
  const requestedRole = session.activeRole;
  const active = isRole(requestedRole) && cu.roles.includes(requestedRole) ? requestedRole : null;

  if (requestedRole && !active) {
    const { activeRole: _stale, ...rest } = session;
    writeSession(res, rest);
  }

  sendData(res, { user: cu.user, roles: cu.roles, activeRole: active }, UserinfoDataSchema);
});
