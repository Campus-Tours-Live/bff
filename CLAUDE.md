...
| **Adding/changing a BFF-owned endpoint (`/auth/*` or `/v1` aggregation) or its OpenAPI spec** | Follow `docs/openapi-conventions.md` — register via the `apiRoute`/`enveloped`/`problemXXX` DSL in `src/openapi/`, keep field descriptions + enums + per-status examples, then run `npm run openapi:lint` + `npm test` (CI's Spectral + contract/drift test enforce it) |
